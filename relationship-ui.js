(function(){
  ...
  function render(id,push=true){
    ...
    card.innerHTML=`<div class="button-row" style="justify-content:space-between"><button class="ghost-btn" type="button" data-rel-back ${stack.length<2?'disabled':''}>← Back</button><button class="dialog-close" type="button" data-rel-close aria-label="Close">×</button></div><p class="eyebrow">${esc(e.region||'upper quarter')} · ${esc(e.type||'structure')}</p><h2>${esc(e.name)}</h2><p>${esc(e.summary||'Curated relationship node.')}</p><div class="section-title"><h3>Why this connects</h3><p>Connections guide comparison and exploration; they do not establish a diagnosis.</p></div>${relationRows(id)}${sources.length?`<div class="fact"><strong>Sources</strong>${sources.map(s=>`<p class="source">${esc(s.publisher)} — ${esc(s.title)}</p>`).join('')}</div>`:''}`;
  ...
  function enhance(){
    ...
    wrap.querySelector('[data-explore-rel]').onclick=()=>{stack=[];render(id,true);dialog.showModal()};
  }
