...
test('verified Scalene reference overlay loads canonical attachments, neural relationships, and Travell provenance',async({page})=>{
  ...
  const data=await page.evaluate(()=>{
    ...
    return {origin:m?.origin,insertion:m?.insertion,innervation:m?.innervation,anterior:m?.attachmentDetail?.anterior,middle:m?.attachmentDetail?.middle,posterior:m?.attachmentDetail?.posterior,visualRelationships:m?.visualRelationships,sourceIds:m?.sourceIds,anatomyValidationStatus:m?.anatomyValidationStatus,anatomyVisualStatus:m?.anatomyVisualStatus,referralSourceIds:r?.sourceIds,referralValidationStatus:r?.validationStatus,referralArtworkStatus:r?.artworkStatus};
  });
